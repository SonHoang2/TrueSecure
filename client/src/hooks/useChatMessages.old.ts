...
export const useChatMessages = ({
    ...
    const sendPrivateImage = async (file: File) => {
        try {
            if (!userKeys.publicKey) {
                console.error('Public key is not available!');
                return;
            ...
            // Use the unified encryption function for both text and binary
            const { encryptedContent, iv, ephemeralPublicKey } =
            ...
            const messageData = {
                ...
                senderId: userId,
                conversationId: conversationId,
                content: objectUrl, // Use object URL for local display
            ...
            const encryptedMessage = {
                ...messageData,
                content: encryptedContent, // Send encrypted binary data
                iv: iv,
                ephemeralPublicKey: ephemeralPublicKey,
                receiverId: chatState.receiver?.id,
            };
    ...
    const sendGroupImage = async (file: File) => {
        try {
            const groupkey = await getGroupKey({
                conversationId,
                userKeys,
            ...
            // Use ArrayBuffer instead of base64 for better performance
            const fileData = await file.arrayBuffer();

            // Use the unified encryption function for both text and binary
            const { encryptedContent, iv } = await cryptoUtils.encryptGroupData(
                groupkey,
                fileData,
            );
    ...
    const sendImage = async (file: File) => {
        try {
            ...
            if (chatState.conversation.isGroup) {
                messageData = await sendGroupImage(file);
            } else {
                messageData = await sendPrivateImage(file);
            }
    ...
    useEffect(() => {
        ...
        socket.on(
            ...
            async (
                ...
                try {
                    ...
                    // Decrypt the binary file data
                    const decryptedArrayBuffer =
                        await cryptoUtils.decryptPrivateData(
                            userKeys.privateKey,
                            {
                                content: data.content,
                                iv: data.iv,
                    ...
                    await storeMessagesInIndexedDB({
                        ...
                        senderId: data.senderId,
                        conversationId: data.conversationId,
                        content: imageUrl, // Store blob URL for display
                        createdAt: data.createdAt,
                        status: data.status,
                        type: data.type,
                    ...
                    if (isCurrentConversation) {
                        const message = {
                            ...data,
                            content: imageUrl, // Use blob URL for display
                            iv: null,
                            ephemeralPublicKey: null,
                        };
        ...
        socket.on('new-group-image', async (data: GroupMessageProps) => {
            try {
                const groupkey = await getGroupKey({
                    conversationId,
                    userKeys,
                ...
                // Decrypt the binary file data
                const decryptedArrayBuffer = await cryptoUtils.decryptGroupData(
                    groupkey,
                    {
                        content: data.content,
                        iv: data.iv,
        ...
        return () => {
            ...
            socket.off('new-private-image');
            socket.off('new-group-image');
        };
